...
def plot_rewards(rewards, map_rewards_goal_rates):
    ...
    plt.plot(rewards_smooth, label='Moving Average of Total Rewards', color='orange')
    plt.xlabel('Episode')
    plt.ylabel('Sum of Rewards')
    plt.title('Moving Average of Total Rewards in Each Episode')
    plt.legend()
    plt.grid(True)
    plt.show()

    #Plotting the rewards for each map
    for map_key, data in map_rewards_goal_rates.items():
        map_rewards = data['rewards']
        map_rewards_smooth = np.convolve(map_rewards, np.ones(window_size) / window_size, mode='valid')
        plt.plot(map_rewards_smooth, label=f'Map {map_key}')
